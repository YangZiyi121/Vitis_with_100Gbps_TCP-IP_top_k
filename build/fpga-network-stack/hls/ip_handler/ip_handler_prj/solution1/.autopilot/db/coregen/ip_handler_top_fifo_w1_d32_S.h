...
// ==============================================================
#ifndef ip_handler_top_fifo_w1_d32_S_HH_
#define ip_handler_top_fifo_w1_d32_S_HH_

#include <systemc>
using namespace std;
SC_MODULE(ip_handler_top_fifo_w1_d32_S) {
    static const unsigned int DATA_WIDTH = 1;
    static const unsigned int ADDR_WIDTH = 6;
    static const unsigned int ip_handler_top_fifo_w1_d32_S_depth = 33;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[ip_handler_top_fifo_w1_d32_S_depth];
    ...
    SC_CTOR(ip_handler_top_fifo_w1_d32_S) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < ip_handler_top_fifo_w1_d32_S_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (ip_handler_top_fifo_w1_d32_S_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < ip_handler_top_fifo_w1_d32_S_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (ip_handler_top_fifo_w1_d32_S_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > ip_handler_top_fifo_w1_d32_S_depth) {
...
#endif //ip_handler_top_fifo_w1_d32_S_HH_
